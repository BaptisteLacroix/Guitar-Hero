...
function interfaceGameInEasy.keypressed(myKey)
    if myKey == "q" then
...
function interfaceGameInEasy.keyreleased(key)
    if key == "q" then
        core.q = 0
    end
    if key == "s" then
...
function interfaceGameInEasy.draw()
...
function interfaceGameInEasy.update(dt)
    dt = math.min(dt, 1/60)
